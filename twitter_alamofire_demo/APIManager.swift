...
class APIManager: SessionManager {
    ...
    static let consumerKey = "4vHkZgfy2kGBYDFcGQeSsceYG"
    static let consumerSecret = "DehlkoHyLv2FyLDGLoWQhJqK6fCW1AIWdSEtIP90l1W8L82tyN"
    ...
    func getHomeTimeLine(maxId: Int64, completion: @escaping ([Tweet]?, Error?) -> ()) {
...
//        }
    ...
    func favorite(_ tweet: Tweet, completion: @escaping (Tweet?, Error?) -> ()) {
        let urlString = "https://api.twitter.com/1.1/favorites/create.json?tweet_mode=extended"
    ...
    func unfavorite(_ tweet: Tweet, completion: @escaping (Tweet?, Error?) -> ()) {
        let urlString = "https://api.twitter.com/1.1/favorites/destroy.json?tweet_mode=extended"
        let parameters = ["id": tweet.id]
        request(urlString, method: .post, parameters: parameters, encoding: URLEncoding.queryString).validate().responseJSON { (response) in
            if response.result.isSuccess,
                let tweetDictionary = response.result.value as? [String: Any] {
    ...
    func retweet(_ tweet: Tweet, completion: @escaping (Tweet?, Error?) -> ()) {
        let urlString = "https://api.twitter.com/1.1/statuses/retweet/\(tweet.id).json?tweet_mode=extended"
        print("sending retweet request to: \(urlString)")
    ...
    func undoRetweet(_ tweet: Tweet, completion: @escaping (Tweet?, Error?) -> ()) {
        let urlString = "https://api.twitter.com/1.1/statuses/unretweet/\(tweet.id).json?tweet_mode=extended"
        request(urlString, method: .post, parameters: nil, encoding: URLEncoding.queryString).validate().responseJSON { (response) in
            if response.result.isSuccess,
                let tweetDictionary = response.result.value as? [String: Any] {
                let tweet = Tweet(dictionary: tweetDictionary)
